...
public class ControllerGUI implements Initializable {
   ...
   @FXML private TableColumn exerciseColumn;
    ...
    private  TextMenu intialiseTableView() {
        ...
        IStatement exRepeat = new CompoundStatement(new VariableDeclarationStatement("v", new IntType()), new CompoundStatement(
                new VariableDeclarationStatement("x", new IntType()), new CompoundStatement(new VariableDeclarationStatement("y", new IntType()), new CompoundStatement(
                        new AssignmentStatement("v",new ConstantExpression(new IntValue(0)) ), new CompoundStatement(
                                new RepeatUntilStatement(new CompoundStatement(new ForkStatement(new CompoundStatement(
                                        new PrintStatement(new VariableExpression("v")), new AssignmentStatement("v",new ArithmeticalExpression(new VariableExpression("v"), new ConstantExpression(new IntValue(1)), '-'))
                                )),new AssignmentStatement("v",new ArithmeticalExpression(new VariableExpression("v"), new ConstantExpression(new IntValue(1)), '+')) ), new RelationalExpression(new VariableExpression("v"), new ConstantExpression(new IntValue(3)), "=="))
        ,
                new CompoundStatement(new AssignmentStatement("x", new ConstantExpression(new IntValue(1))), new CompoundStatement(new NoStatement(), new CompoundStatement(
                       new AssignmentStatement("y", new ConstantExpression(new IntValue(3))), new CompoundStatement(
                               new NoStatement(), new PrintStatement(new ArithmeticalExpression(new VariableExpression("v"), new ConstantExpression(new IntValue(10)), '*'))
                )
                ))))
        ))
        ));
...
*/
    ...
    private void clearfiles() {
        ...
        {
            try {
                PrintWriter writer = new PrintWriter("log"+i.toString()+".txt");
                writer.close();
