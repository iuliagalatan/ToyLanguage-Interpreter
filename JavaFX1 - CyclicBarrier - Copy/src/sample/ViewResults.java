...
public class ViewResults implements Initializable {
    ...
    @FXML private TableColumn ListOfValuesColumn;
    @FXML private TableView Barrier;
    ...
    private final ObservableList<DataModel> BarrierData =  FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        LocationColumn.setCellValueFactory(new PropertyValueFactory<>("key"));
        ValueColumn.setCellValueFactory(new PropertyValueFactory<>("value"));
        ListOfValuesColumn.setCellValueFactory(new PropertyValueFactory<>("list"));
        Barrier.setItems(BarrierData);
    ...
    private void updateBarrierTable() {
        ...
        HashMap<Integer, Pair<Integer, List<Integer>>> hm = getCurrentProgramState().getBarrierTable().getHeap();

        BarrierData.clear();
        if (hm.size() == 0)
            return;
        for (Map.Entry<Integer, Pair<Integer, List<Integer>>> entry : hm.entrySet()) {
            List<String> newlist = new ArrayList<>(entry.getValue().getValue().size());
            for ( Integer myint : entry.getValue().getValue())
                newlist.add(String.valueOf(myint));


            BarrierData.add(new DataModel(entry.getKey().toString(), entry.getValue().getKey().toString(), newlist));
        }
    ...
    public static class DataModel {
        ...
        public DataModel(String key, String value, List<String> list) {
            this.key = new SimpleStringProperty(key);
            this.value = new SimpleStringProperty(value);
            this.list = new SimpleStringProperty(String.join(" ", list));
        }
