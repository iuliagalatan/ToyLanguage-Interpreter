...
public class ControllerGUI implements Initializable {
   ...
   @FXML private TableColumn exerciseColumn;
    ...
    private  TextMenu intialiseTableView() {
        ...
        IStatement exSemaphore = new CompoundStatement(new VariableDeclarationStatement("v1", new RefType(new IntType())),
        new CompoundStatement(new VariableDeclarationStatement("cnt", new IntType()), new CompoundStatement(
                new NewStatement("v1", new ConstantExpression(new IntValue(1))), new CompoundStatement(
                        new CreateSemaphore("cnt", new ReadHeapExpression(new VariableExpression("v1"))),
                new CompoundStatement(new ForkStatement(new CompoundStatement(new aquire("cnt"), new CompoundStatement(new WriteHeapStatement("v1", new ArithmeticalExpression(
                        new ReadHeapExpression(new VariableExpression("v1")), new ConstantExpression(new IntValue(10)), '*')
                ), new CompoundStatement(new PrintStatement(new ReadHeapExpression(new VariableExpression("v1"))), new release("cnt")) ))),
                        new CompoundStatement(new  ForkStatement(new CompoundStatement(new aquire("cnt"), new CompoundStatement(new WriteHeapStatement("v1", new ArithmeticalExpression(
                                new ReadHeapExpression(new VariableExpression("v1")), new ConstantExpression(new IntValue(10)), '*')),new CompoundStatement(
                                new WriteHeapStatement("v1", new ArithmeticalExpression(
                                new ReadHeapExpression(new VariableExpression("v1")), new ConstantExpression(new IntValue(2)), '*') ), new CompoundStatement( new PrintStatement(new ReadHeapExpression(new VariableExpression("v1"))),  new release("cnt")

        ))))), new CompoundStatement(new aquire("cnt"), new CompoundStatement(new PrintStatement(new ArithmeticalExpression(new ReadHeapExpression( new VariableExpression("v1")), new ConstantExpression(new IntValue(1)), '-')),new release("cnt") ))
                        ) )))));

        if (TypeChecker(exSemaphore)) {System.out.println("nu merge typecheck for"); return null;}
        List<ProgramState> prg16 = new ArrayList<>();
        prg16.add(new ProgramState(exSemaphore));
        ...
        menu.addCommand(new RunCommand("16", exSemaphore.toString(), ctrl16));
